...
export function FloatingDock() {
  ...
  return (
    <>
      <header className={`fixed top-4 inset-x-0 z-50 flex justify-center px-3 sm:px-6 transition-all duration-300 pointer-events-none ${scrolled ? "translate-y-0" : "translate-y-0"}`}>
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-x-4 top-24 z-50 rounded-3xl p-6 shadow-2xl md:hidden animate-in fade-in slide-in-from-top-3 duration-200 glass-pill">
          <nav className="flex flex-col gap-4 text-xs font-bold text-foreground">
            <a href="#vsl" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-2.5 p-2.5 rounded-xl hover:bg-white/5 transition-colors">
              <Play className="h-3.5 w-3.5 text-accent fill-current" /> Executive Briefing
            </a>
            <a href="#calculator" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-2.5 p-2.5 rounded-xl hover:bg-white/5 transition-colors">
              <Calculator className="h-3.5 w-3.5 text-accent" /> Pipeline Bleed Calculator
            </a>
            <a href="#leaks" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-2.5 p-2.5 rounded-xl hover:bg-white/5 transition-colors">
              <ShieldAlert className="h-3.5 w-3.5 text-accent" /> The 5 Broken Handshakes
            </a>
            <a href="#system" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-2.5 p-2.5 rounded-xl hover:bg-white/5 transition-colors">
              <Layers className="h-3.5 w-3.5 text-accent" /> The Duxio Architecture
            </a>
            <a href="#faq" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-2.5 p-2.5 rounded-xl hover:bg-white/5 transition-colors">
